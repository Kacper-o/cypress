                    ...
                    <div className="flex min-h-full items-center justify-center p-4 text-center">
                        ...
                        >
                            <Dialog.Panel className="relative w-full max-w-lg max-h-[90vh] overflow-y-auto transform rounded-2xl bg-white dark:bg-gray-800 p-6 text-left shadow-xl transition-all flex flex-col gap-5">
                                <button
                                    type="button"
                                    className='absolute top-2 right-2 z-10 w-fit p-2 bg-primary-blue-100 dark:bg-black rounded-full'
                                    onClick={closeModal}
                                >
                                    <XMarkIcon width={20} height={20} className='object-contain text-black dark:text-gray-200' />
                                </button>
                                <div className='flex-1 flex flex-col gap-3'>
                                    ...
                                    <div className="flex gap-3">
                                        <div className="flex-1 relative w-full h-24 bg-primary-blue-100 dark:bg-gray-600 rounded-lg">
                                    ...
                                    <div className="mt-3 flex flex-wrap gap-4">
                                        {Object.entries(car).map(([key, value]) => (
                                            <div className='flex justify-between gap-5 w-full text-right' key={key}>
                                                <h4 className='text-grey dark:text-gray-400 capitalize'>{key.split("_").join(" ")}</h4>
                                                <p className='text-black-100 dark:text-gray-200 font-semibold'>{value}</p>
                                            </div>
                                        ))}
                                    </div>
                                </div> 
